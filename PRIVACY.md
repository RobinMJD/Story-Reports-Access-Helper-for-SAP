...
These checks are scheduled by page mutations rather than continuous polling. Observation remains active for the lifetime of that IAS document so an exact SAP structure that appears after a slow load can still be detected. The observer and its pending detection timers are stopped after the document has been reported once.
...
Microsoft Edge does not retroactively inject a newly installed or re-enabled static content script into a document that was already open. For an active, safe, non-private tab on the exact supported Report Center path, the extension checks whether its current-build marker is present. A matching tab may be checked and refreshed while Edge still reports it as loading. The extension performs this bounded check after installation, on a same-build service-worker start (including re-enablement), on tab activation, and when Microsoft Edge reports a matching URL change or page-load completion. If the marker is absent, it validates the tab again, records one attempt, and performs one ordinary refresh with normal cache behavior.
...
The recorded state proves only that one continuation was authorized. It does not prove network delivery, authentication success, or Story rendering. The exact-document response is time-bounded so a stale browser message channel cannot keep recovery pending indefinitely. If the browser setting is ineffective, enterprise policy blocks it, the source document changes, the bounded response expires without durable commit evidence, or the durable barrier cannot be completed, the extension stops without retrying or broadening access.

The popup status **Access fix applied** means either that an exact extension-owned browser setting is currently verified as effective for the active SuccessFactors site or that the extension durably recorded the local one-use continuation step for that tab. It does not mean that SAP authentication, authorization, network delivery, or Story rendering succeeded.
