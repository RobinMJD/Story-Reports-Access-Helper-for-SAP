...
const DEFAULT_EDGE_CERTIFICATION_NOTES = [
  ...
  "On the exact active Report Center path only, a DOM-free current-build marker answers a local probe and sends only build/protocol readiness. The active page is evaluated after installation, on a same-build service-worker start including re-enablement, on tab activation, and on Microsoft Edge URL-change or page-completion events. One trusted-local marker stores only the current extension build/version so a same-build worker start can be distinguished from a version transition; a version transition skips the immediate generic startup scan. A session write-ahead record permits at most one ordinary cache-preserving automatic refresh without inspecting report content, refreshing background or unrelated tabs, or changing focus.",
  "The popup first requests a contextual status and capability result. Fix this report is hidden when the page is unsupported, loading, prepared, working, or already fixed; if the availability check fails, the popup may display it as a fallback but the service worker still independently revalidates and fails closed. An accepted request is durably recorded, its result is returned for display, and then at most one cache-preserving refresh occurs. A 30-second repeat guard prevents rapid repeated refreshes. The action never clears browser cookies, the allowance ledger, or unrelated settings.",
